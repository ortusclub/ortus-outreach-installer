...
try {
  ...
  await page.evaluate(() => {
    const item = { id: 'h-selected-fixture', histIdx: 0, name: 'Selected historical campaign', where: 'local', bucket: 'done',
    ...
    if (action !== "openLocalHistoryCampaign('h-selected-fixture')") throw new Error('Expanded card must target selected history');
    window.openLocalHistoryCampaign(item.id);
  });
