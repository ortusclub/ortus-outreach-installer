...
test('controls: monitoring local → dashRunCheck bulk and separate monitoring stop', () => {
  ...
  assert.match(resume.onclick, /openCampaignContinuation\('local-active','local'\)/);
});
...
test('a cloud monitoring campaign with unsent leads offers Resume sending', () => {
  ...
  assert.equal(resume.once, true);
  assert.match(resume.onclick, /openCampaignContinuation\('c1','vm'\)/,
    'monitoring uses the same continuation choices while preserving its machine');
});
