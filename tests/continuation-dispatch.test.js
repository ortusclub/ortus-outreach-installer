...
function fixture(choice, { changed = false, vmMode = false, historyExecution } = {}) {
  ...
  const status = { id: vmMode ? 'cloud-a' : 'legacy-singleton', executionId: 'run-a', mode: 'connect_and_introduce',
    state: 'idle', status: 'cancelled', name: 'QA', profileIds: ['sender'], sheetUrl: 'sheet',
...
test('local choices dispatch to separate commands; cancel makes no mutation', async () => {
  ...
  for (const choice of [null, ...Object.keys(expected)]) {
    ...
    if (choice === 'check') { assert.equal(f.calls[0][1].continuationCheck, true); assert.deepEqual(f.calls[0][1].profileIds, ['sender']); }
  }
});
