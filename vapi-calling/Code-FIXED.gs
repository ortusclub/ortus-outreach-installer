 ...
 * ✅ AUTO FOLLOW-UP after Voicemail/No Answer
 * ✅ Webhook idempotency (duplicate protection)
 * ✅ Concurrent webhook locking (race condition fix)
 *
...
// ============================================================
// FIX CR-03: Cleaned header aliases — removed ambiguous mappings
// that caused 'city' to match both location and event_city.
// Each alias now uniquely identifies one column.
// ============================================================
...
function scheduleAutoFollowup_(sheetId, sheetName, rowNum, status, prospectData) {
  ...
  if (config.specificTime) {
    const timeParts = config.specificTime.split(':');
...
function sendAutoFollowup_(item, config) {
  ...
  const channels = item.channels || config.channels;
    ...
    if (wasMessagedThisWeek_(vars.phone)) {
      ...
      try {
        ...
        if (sid && token && fromNumber) {
          ...
          const batchId = prospectData.batchId || 'auto_followup';
...
// ============================================================
// PROCESS QUEUE — FIX CR-04: Removed duplicate budget check
// ============================================================
...
function processQueueForSheet_(ss, sh, apiKey, fallbackPhoneNumberId, fallbackAssistantId, concurrency) {
  ...
  try {
    ...
    // Load MULTI_ROUND_ configs for this sheet
    const batchConfigs = {};
    const allProps = sp.getProperties();
    for (const key in allProps) {
    ...
    // Also load SECOND_ROUND_ configs (backward compatibility)
    for (const key in allProps) {
    ...
    // FIX CR-04: Single budget check (was duplicated before)
    if (!isUnderBudget_()) {
    ...
    while (Object.keys(active).length < concurrency && queue.length > 0) {
      ...
      const baseBatchId = rowBatchId.replace(/_R\d+$/, '');
...
// ============================================================
// WEBHOOK (doPost) — FIX CR-01 + CR-05
// Locked active map, validated payload, idempotency
// ============================================================
...
function doPost(e) {
  ...
  try {
    ...
    // ---- Process call data (row-specific writes — safe without lock) ----
    ...
    // ============================================================
    // CR-01 FIX: LOCK the shared active/queue state
    // Two concurrent webhooks could corrupt the active map without this.
    // processQueueForSheet_ has its own lock, so we release ours first.
    // ============================================================
    ...
    if (!hasLock) {
      Logger.log('WARNING: doPost could not acquire lock after 15s for call ' + callId);
    }
    ...
    // Mark webhook as processed (idempotency for VAPI retries)
    ...
    // Return 200 so VAPI doesn't keep retrying a broken webhook
    return ContentService.createTextOutput('OK');
  }
}
...
// ============================================================
// PASTE POINT — See instructions below
// ============================================================
//
// >>> PASTE YOUR EXISTING FUNCTIONS HERE <<<
//
// From your CURRENT Code.gs, copy EVERYTHING between these two markers:
//
//   START copying from: function vapiScheduleHighlighted() {
//   STOP copying before: function startSecondRoundAutomatically_(config) {
//
// That block contains all your dialog wizards, campaign functions,
// multi-round system, cancel/view functions, sort, budget dialog,
// and debug functions — ALL UNCHANGED.
//
// The functions BELOW this point (startSecondRoundAutomatically_,
// autoSortIfBatchComplete_, etc.) are the FIXED versions and must
// stay as they are.
// ============================================================

// ============================================================
// FIX CR-02: startSecondRoundAutomatically_ — uses queue now
// Previously called triggerVapiCall_ directly, bypassing concurrency.
// ============================================================
...
function startSecondRoundAutomatically_(config) {
  ...
  // CR-02 FIX: Queue rows through the standard system instead of direct API calls
...
// ============================================================
// AUTO-SORT — FIX WR-05: Added lock to prevent concurrent sorts
// ============================================================
...
function vapiWatchdog_() {
  ...
  for (const sh of sheets) {
    ...
    if (queue.length > 0 && Object.keys(active).length < concurrency) {
      const apiKey = sp.getProperty('VAPI_API_KEY');
      if (apiKey) {
        updateStatus_({ 'Script Status': '🔄 Watchdog resuming...' }, sheetName);
        processQueueForSheet_(ss, sh, apiKey, sp.getProperty('VAPI_PHONE_NUMBER_ID'), sp.getProperty('VAPI_ASSISTANT_ID'), concurrency);
      }
    }
...
function addToDailySpend_(amount) {
  const sp = PropertiesService.getScriptProperties();
  const current = Number(sp.getProperty(getDailySpendKey_()) || 0);
...
function showBudgetWarning_(threshold, spent, budget) {
  ...
  const percentUsed = (spent / budget) * 100;
  Logger.log((threshold >= 90 ? '🚨' : '⚠️') + ' BUDGET WARNING: $' + spent.toFixed(2) + ' / $' + budget + ' (' + percentUsed.toFixed(1) + '%)');
  try {
    const ui = SpreadsheetApp.getUi();
    ui.alert('Budget at ' + threshold + '%', 'Spent: $' + spent.toFixed(2) + ' / $' + budget + '\nRemaining: $' + remaining.toFixed(2), ui.ButtonSet.OK);
  } catch (e) {
...
function getCallCostFromVapi_(apiKey, callId) {
  try {
    const res = UrlFetchApp.fetch('https://api.vapi.ai/call/' + callId, {
      method: 'get',
...
function logToProjectTagSheet_(toNumber, projectTag) {
  ...
  const cleanNumber = String(toNumber).replace(/[^0-9]/g, '');
  const lastRow = logSheet.getLastRow() + 1;
  logSheet.getRange(lastRow, 2).setValue(cleanNumber);
  logSheet.getRange(lastRow, 4).setValue(projectTag);
  logSheet.getRange(lastRow, 5).setValue(new Date());
}
...
// Run this on a daily time-based trigger to clean old keys.
// ============================================================
